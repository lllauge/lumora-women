  ...
  const sections = [
    {
      name: 'whatISaw',
      label: 'What I saw',
      placeholder: `e.g. You hit protein 5 of 7 days and your weight trend is moving — the two days that slipped were both weekends.`,
      initial: initialReview?.what_i_saw ?? '',
    },
    {
      name: 'whatChanged',
      label: `What we're changing`,
      placeholder: `e.g. I moved more of your carbs to breakfast because your energy dipped mid-morning — your menu is updated.`,
      initial: initialReview?.what_changed ?? '',
    },
    {
      name: 'focus',
      label: 'Your focus this week',
      placeholder: `e.g. One thing only: protein at breakfast before coffee. Everything else is already working.`,
    ...
    <form action={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button type="submit" className="admin-btn-primary" disabled={pending}>
          {pending ? 'Saving…' : initialReview ? 'Update this week’s review' : `Send to ${clientFirstName}’s Today page`}
        </button>
