...
export async function sendClientLoginCode(input: {
  to: string
  code: string
}) {
  const resendKey = process.env.RESEND_API_KEY
  ...
  const { error } = await resend.emails.send({
    from: 'Lumora Women Security <hello@lumorawomen.com>',
    to: input.to,
    subject: `${input.code} is your Lumora Women login code`,
    html: `
      <div style="font-family:Arial,sans-serif;background:#F8F6F0;padding:32px">
        <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:16px;padding:36px;border:1px solid #E5E0D6">
          <h1 style="font-family:Georgia,serif;color:#1A2818;margin:0 0 16px;font-size:28px">Your login code</h1>
          <p style="color:#3A4A38;font-size:16px;line-height:1.7">Enter this code on the login page to sign in without your password:</p>
          <div style="font-size:36px;font-weight:800;letter-spacing:10px;color:#3F6936;margin:24px 0">${input.code}</div>
          <p style="color:#6B6B64;font-size:13px;line-height:1.6">This code can be used once and expires after a short time. If you did not request it, you can safely ignore this email — no one can sign in without the code.</p>
        </div>
      </div>
    `,
    text: `Your Lumora Women login code is ${input.code}. Enter it on the login page to sign in without your password. It can be used once and expires after a short time. If you did not request it, you can safely ignore this email.`,
  })
  return error ? { ok: false, error: error.message } : { ok: true, error: null }
}
